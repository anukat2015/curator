...
def curate_new(num_to_keep)
  ...
  ey_data = Sorter.new(data: company_data, metric: :earnings_yield, num_to_keep: num_to_keep)
  roc_data = Sorter.new(data: company_data, metric: :return_on_capital, num_to_keep: num_to_keep)
  ReportCreator.new(data: ey_data, type: CompanyReportByEarningsYield)
  ReportCreator.new(data: roc_data, type: CompanyReportByReturnOnCapital)
